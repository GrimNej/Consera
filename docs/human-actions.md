...
| H-02 | Confirm LingoQL $20 credit and billing protections                                 | Requires account access; payment/overage settings are security/financial controls | waiting_for_user | 2026-07-18   | —                    | pending sanitized user confirmation                |
...
```yaml
checkpoint: H-02
git_commit_at_marker_creation: 35fe21f
completed_tests:
  - pnpm check
  ...
  - pnpm build
  - pnpm cost:preflight (expected failure before H-02)
next_command: pnpm cost:preflight
expected_files:
  - docs/cost-record.json
  - docs/platform-evidence/<timestamp>_H-02-lingoql-credit.md
verification_criterion: cost preflight passes with at least $20 credit, no payment method, and automatic overage disabled
```
