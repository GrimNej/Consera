...
| H-03 | Create/select Consera LingoQL project and connect repository                       | Requires LingoQL account authorization                                            | blocked      | 2026-07-18   | —                    | Runtime proves `0.0.0.0:8080` readiness; a 512 MB test still returned `502`; service stopped and platform routing diagnosis is required |
...
```yaml
checkpoint: H-03
git_commit_at_marker_creation: b574a41
completed_tests:
  - pnpm check
  ...
  - pnpm web:smoke
  - pnpm cost:preflight
next_command: pnpm lingoql:verify-h03 -- <health_url>
expected_files:
  - docs/platform-evidence/<timestamp>_H-03-lingoql-health.md
  - docs/platform-connection-checklist.md
verification_criterion: HTTPS health route returns the exact Consera health response after the repository-backed no-secret deployment
```
